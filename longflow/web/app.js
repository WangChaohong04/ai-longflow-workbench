...
function buildGeoCard({ geo, title, key }, geoKey) {
  ...
  buildMapPane(mapPane, geo, geoKey + ":" + (key || "x"));
  buildSiteList(listPane, geo);
  return card;
}
...
function exitRoute(panel, geo) {
  panel.mode = "normal";
  panel.routeTarget = null;
  panel.routeResult = null;
  try { panel.plugin.exitRoute(); } catch (e) { /* ignore */ }
  try { panel.plugin.showSites(geo, (cc) => { panel.selected = cc; renderMapBottom(panel, geo); }); } catch (e) { /* ignore */ }
  renderMapBottom(panel, geo);
}
...
function renderRouteBottom(panel, geo, bottom) {
  ...
  for (const m of (panel.plugin.modes || [])) {
    seg.appendChild(el("button", {
      ...
      onClick: () => { panel.routeMode = m.id; panel.routeResult = { loading: true }; renderMapBottom(panel, geo); runRoute(panel, geo); },
    }, m.label));
  }
